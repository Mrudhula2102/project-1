...
const Login = () => {
    ...
    const savedata =()=> {
      ...
      console.log(inputs)
      axios.get("http://localhost:4005/logins",inputs)
      .then((response)=>{
    ...
    const handleSubmit = (event) => {
      ...
      var { hrid, password } = document.forms[0];
  
      // Find user login info
      const userData = database.find((user) => user.hrid === hrid.value);
      ...
      {
          setError({ name: "hrid", message: errors.hrid });
      }
    };
  ...
  return (
    ...
    <ThemeProvider  theme={defaultTheme} >
      ...
      <h1 align='center'>ADMIN LOGIN</h1>
       <TextField  margin="normal" required fullWidth id="HR ID" label="HR ID" name="hrid" 
                autoComplete="id"  autoFocus onChange={inputHandler} value={inputs.hrid}/><br/>

       

      <TextField  margin="normal" required fullWidth name="password" label="Password" type="password" id="password" 
                autoComplete="current-password"  onChange={inputHandler} value={inputs.password}/><br/>
